...
<template>
  <el-tabs v-model="activeName">
    <el-tab-pane label="借阅审核" name="first">
      ...
      <el-table
        ...
        <el-table-column prop="borrowStatus" label="状态">
          <template slot-scope="scope">
              <div v-if="scope.row.borrowStatus === 0">待审核</div>
          </template>
        </el-table-column>
        <el-table-column prop="createDate" label="上传时间"  />
        <el-table-column label="编辑" width="160" fixed="right">
          <template slot-scope="scope">
            <el-button type="primary" size="mini" @click="passBooksMessage(scope.row, 1)">通过</el-button>
              <el-button type="danger" size="mini" @click="rejectBooksMessage(scope.row, 1)">拒绝</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-tab-pane>
    <el-tab-pane label="待归还" name="second">
      <el-button type="primary" size="mini" @click="reFindBooksMessage">重新查询</el-button>
      <el-table
        :data="bookList"
        height="65vh"
        ...
        <!-- <el-table-column prop="uploader" label="上传者" width="150" /> -->
        <el-table-column prop="classification" label="类别" />
        <el-table-column prop="content" label="拒绝理由" width="150">
          <template slot-scope="scope">
            <el-input
              placeholder="请输入内容"
        ...
        <el-table-column prop="status" label="状态">
          <template slot-scope="scope">
              <div v-if="scope.row.status === 0">待归还</div>
          </template>
        </el-table-column>
        <el-table-column prop="createDate" label="上传时间"  />
        <el-table-column label="编辑" width="180" fixed="right">
          <template slot-scope="scope">
              <el-button type="primary" size="mini" @click="passBooksMessage(scope.row, 0)">确认归还</el-button>
              <el-button type="danger" size="mini" @click="rejectBooksMessage(scope.row, 0)">拒绝</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-tab-pane>
...
<script>
import { getBooksBorrow, updataBorrowData, deleteBooksBorrow, passBooksMessage } from '../../api/book'
...
export default {
  ...
  methods: {
     ...
     * @param {*} type 
     */
    async rejectBooksMessage(row, type) {
      let dataObj = {}
      if (type === 1) { // 借阅
        dataObj = {
          id: row.id,
        ...
        try {
          await updataBorrowData(dataObj)
          this.$message({
            type: 'success',
            message: '拒绝成功!'
          })
        ...
        try {
          await passBooksMessage(dataObj)
          this.$message({
            type: 'success',
            message: '拒绝成功!'
          })
     ...
     * 通过审核
     * @param {*} row 
     * @param {*} type 1 借阅审核   0 归还审核
     */
    async passBooksMessage(row, type) {
      console.log('---------row', row);
      let dataObj = {}
      if (type === 1) { // 借阅
        dataObj = {
